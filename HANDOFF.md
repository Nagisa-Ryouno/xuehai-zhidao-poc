...
> **Demo 状态与双端数据表现说明**：
> - **Student Web（学生端）**：默认首次进入为“新用户 Demo 状态”（0 掌握度历史、0 错题、0 预设教师建议、今日安排提示“开始你的第一次学习”），支持评委与用户从零建立个人学情档案并体验自适应导学；
> - **Teacher Web（教师端）**：保留预设 S001~S005 完整班级演示数据，用于宏观班级学情看板、30 知识点薄弱度诊断与教师轻量干预动作全流程展示。
...
3. **架构规模边界**：
   - 本系统为中国国际大学生创新大赛（2026）参赛项目技术原型（POC），底层采用轻量原子文件替换持久化（`data/*.json`, `data/*.jsonl`），适合单机/演示/教学试验场景，不适用于数千并发的高吞吐生产环境。
