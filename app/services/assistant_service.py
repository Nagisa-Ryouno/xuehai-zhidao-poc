...
学海智导 (Xuehai Zhidao) V2 AI 学习助手核心应用服务模块 (Assistant Service)

设计架构：
ProfileRepository (学生数据仓储)
       ↓
Context Builder (学生多维学情上下文提炼)
       ↓
LLM Client (大模型调用、超时与异常处理)
       ↓
...
from app.infrastructure.external.llm_client import llm_client
...
def build_student_context(student_id: str) -> Optional[Dict[str, Any]]:
    ...
    reports = ProfileRepository.get_student_reports()
...
class AssistantService:
    ...
    def build_student_context(self, student_id: str) -> Optional[Dict[str, Any]]:
        return build_student_context(student_id)
    ...
    def generate_assistant_response(
        ...
        if self.client.is_available():
            ...
            llm_raw_result = self.client.generate(
            ...
            if llm_raw_result:
                ...
                if validated:
                    print(f"[Assistant] provider={self.client.get_provider_name()} status=success")
    ...
    def get_assistant_greeting(self, student_id: str) -> Dict[str, Any]:
...
# 模块级兼容函数
generate_assistant_response = assistant_service.generate_assistant_response
get_assistant_greeting = assistant_service.get_assistant_greeting
