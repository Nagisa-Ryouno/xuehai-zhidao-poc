...
学海智导 V2 - Path Replanning Application Service 局部动态重规划应用服务

编排知识图谱只读探针、认知状态仓储、路径执行状态仓储与纯领域决策核心 (evaluate_decision_core)。
"""
...
def get_prerequisites(knowledge_id: str) -> List[str]:
    ...
    return knowledge_graph_service.get_prerequisites(knowledge_id)


def get_successors(knowledge_id: str) -> List[str]:
    """从权威知识图谱中读取知识点的直接后继知识点列表 (Read-Only)"""
    return knowledge_graph_service.get_successors(knowledge_id)
...
def is_knowledge_mastered(
    ...
    try:
        state = default_bkt_state_repository.get_state(
...
def evaluate_and_replan(
    ...
    before_path = default_path_state_repository.get_path_state(
    ...
    if f_after >= 0.80 and successors:
        for succ in successors:
            succ_state = default_path_state_repository.get_path_state(
