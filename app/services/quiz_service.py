...
"""
quiz_service.py
学海智导 (Xuehai Zhidao) V2 分知识点微测验应用服务层 (Quiz Service)

职责：
1. 维护微观经济学分知识点微测验题库 (QuizBankRepository)
2. 提供脱敏的公共题目查询接口（严格禁止向学生暴露正确答案与解析）
3. 执行服务端安全判题，确保选项合法与题目真实性
4. 联动 EventRepository 自动持久化 QUESTION_ATTEMPT 事件
5. 联动 BKT 事件处理器流式演进学生认知状态，保证严格幂等与事实来源不丢失
6. 联动局部动态路径重规划应用服务 (evaluate_and_replan) 触发路径执行状态演进与审计信封生成
"""
...
from app.infrastructure.persistence.quiz_bank_repository import QuizBankRepository
from app.services.knowledge_graph_service import knowledge_graph_service
from app.services.path_replanning_service import DecisionAuditEnvelope, evaluate_and_replan

DEFAULT_QUIZ_FILE = settings.QUIZ_BANK_FILE
...
class QuizSubmitResponse(BaseModel):
    ...
    replanning: Optional[DecisionAuditEnvelope] = Field(
...
def load_quiz_bank(file_path: Optional[Path] = None) -> List[QuizQuestionInternal]:
    ...
    if not target_file.exists():
        fallback = settings.PROJECT_ROOT / "data" / "quiz_bank.json"
        target_file = fallback if fallback.exists() else target_file
    ...
    try:
        ...
        raw_list = json.loads(content) if content else []
    except Exception:
        raw_list = []
...
def submit_quiz_answer(
    ...
    target_events = events_file or getattr(event_service, "DEFAULT_EVENTS_FILE", settings.LEARNING_EVENTS_FILE)
    ...
    target_path_states = path_states_file or getattr(path_state_service, "DEFAULT_PATH_STATES_FILE", settings.LEARNING_PATH_STATES_FILE)

    stored_event = default_event_repository.record_event(event_in, target_file=target_events)

    learning_state: Optional[LearningStateSnapshot] = None
    replanning_envelope: Optional[DecisionAuditEnvelope] = None
    try:
        ...
        if bkt_res.status == "updated" and bkt_res.state is not None:
            ...
            try:
                ...
                replanning_envelope = evaluate_and_replan(
                    ...
                    states_file=target_path_states,
                    bkt_states_file=target_states,
                )
            except Exception:
