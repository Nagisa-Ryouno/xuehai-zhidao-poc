...
**本设计规范（Phase 2.1 终审版）为系统量身定制演进蓝图**：坚决摒弃微服务、消息队列（MQ/Kafka）、Redis、Docker/K8s、复杂数据库等与当前“中国国际大学生创新大赛（2026）/ 概念验证（POC）”定位脱节的过度设计（No Over-Engineering）；以 **`app/` 为顶级命名空间的实用分层单体** 为推荐目标，确立严格的单向依赖规则与公有服务契约，保障未来继续推进 P0-9、P1、P2 时具备清晰的工程底座。
